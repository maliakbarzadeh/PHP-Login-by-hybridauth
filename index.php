				...
				<form class="login100-form validate-form flex-sb flex-w" action="login.php" method="post">
					...
					<?php foreach ($hybridauth->getProviders() as $name) : ?>
						<?php if (!isset($adapters[$name]) & $name=='Facebook') : ?>
							<a href="#" class="btn-face m-b-20" onclick="javascript:auth_popup('<?php print $name ?>');">
								<i class="fa fa-facebook-official"></i>
								Facebook
							</a>
						<?php endif; ?>
						<?php if (!isset($adapters[$name]) & $name=='Google') : ?>
							<a href="#" class="btn-google m-b-20" onclick="javascript:auth_popup('<?php print $name ?>');">
								<img src="images/icons/icon-google.png" alt="GOOGLE">
								Google
							</a>
						<?php endif; ?>
						<?php if (!isset($adapters[$name]) & $name=='Discord') : ?>
							<a href="#" class="btn-google btn-success m-b-20" onclick="javascript:auth_popup('<?php print $name ?>');">
								<img src="images/icons/icon-discord.png" alt="DISCORD">
								Discord
							</a>
						<?php endif; ?>
						<?php if (!isset($adapters[$name]) & $name=='Reddit') : ?>
							<a href="#" class="btn-google m-b-20" onclick="javascript:auth_popup('<?php print $name ?>');">
								<img src="images/icons/icon-reddit.png" alt="Reddit">
								Reddit
							</a>
						<?php endif; ?>
					<?php endforeach; ?>
